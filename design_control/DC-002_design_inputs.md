...
**Document:** DC-002 · Version 2.5 · 2026-06-06  
...
| DI-019-H | UN-019 | The progress bar shall not remain at any single whole-number percentage for more than 1 second due to animation algorithm constraints — enforced by: (a) a minimum per-frame increment floor in the Tick loop, (b) a minimum per-poll advancement floor in PollChromeReady, (c) a PollChromeReady cap ≤ 98 so that `Int()` display can never show "100%" while loading is incomplete, (d) a mathematical proof that the Tick easing can traverse from the cap to the 99.5 close-trigger in < 1000 ms: `(99.5 − cap) ÷ min_floor × 16 ms < 1000 ms`, and (e) a Tick keep-alive branch (`ElseIf Not readyToClose And stepVal < 99.4`) that advances stepVal at the per-frame floor when stepVal ≥ targetVal and loading is incomplete, preventing convergence stall at the animation ceiling; ceiling 99.4 guarantees `Int(stepVal) ≤ 99` until `readyToClose = True`, and traversal rate `1.0 ÷ 0.05 × 16 ms = 320 ms` per percentage point < 1000 ms | `start_splash.hta` Tick floor; PollChromeReady floor + cap ≤ 98; `Int(stepVal)` display; computed `(99.5 − cap) / min_floor * 16 < 1000`; `ElseIf Not readyToClose And stepVal < 99.4` keep-alive with step ≥ 0.05 and ceiling < 99.5 | P2 | OPEN |
