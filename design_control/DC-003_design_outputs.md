...
| Design Output | File | Symbol / Route | Implements |
|---|---|---|---|
| Startup splash HTA | `Athena/ui/start_splash.hta` | `.bar-wrap` CSS; `#pct` + `pctEl` + `float:right`; `PollChromeReady` + `readyToClose`; asymptotic `Tick`; `Int()` display; cap=97; `adv`/`inc` floors; `.name` font-size 101px; `TickDots` + `dots_id` VBScript cycling; `#dots` single-span; `CloseSplash` sub writes `.athena_splash_done` before `window.close()` | DI-019-A, DI-019-B, DI-019-C, DI-019-F, DI-019-H, DI-019-I, DI-019-J, DI-019-L |
| Chrome launch delay | `Athena/ui/start_athena.ps1` | `$splashDoneFile` polling loop (200ms, 6s max) waits for `.athena_splash_done` before launching Chrome | DI-019-G, DI-019-L |
